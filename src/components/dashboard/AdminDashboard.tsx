...
function StatCard({ title, value, icon: Icon, subtitle, color }: {
  ...
  return (
    <Card className="group border-primary/15 bg-gradient-to-br from-card via-card to-primary/10">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-xs font-medium text-muted-foreground">{title}</CardTitle>
        <div className={`rounded-md p-1.5 shadow-[0_0_18px_-6px_currentColor] ${color ? 'bg-current/10' : 'bg-primary/10'}`}>
        ...
        <div className="space-y-2">
          {alerts.data.map((alert, i) => (
            <div key={i} className={`flex items-center gap-3 rounded-lg border p-3 backdrop-blur-sm ${
              alert.type === 'error' ? 'border-destructive/40 bg-destructive/10 shadow-[0_0_18px_-6px_hsl(var(--destructive)/0.35)]' :
              alert.type === 'warning' ? 'border-warning/40 bg-warning/10 shadow-[0_0_18px_-6px_hsl(var(--warning)/0.3)]' :
              'border-border bg-muted/30 shadow-[0_0_14px_-8px_hsl(var(--primary)/0.2)]'
            }`}>
          ...
          <Card className="border-primary/15 bg-gradient-to-br from-card via-card to-primary/10">
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="rounded-full bg-muted/60 p-2 shadow-[0_0_14px_-6px_hsl(var(--muted-foreground)/0.25)]"><WifiOff className="h-5 w-5 text-muted-foreground" /></div>
